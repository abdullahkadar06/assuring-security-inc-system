  ...
  try {
    ...
    const summary = await pool.query(
      ...
      WITH att AS (
        SELECT
          a.user_id,
          COUNT(*) FILTER (WHERE a.status IN ('CLOSED', 'AUTO_CLOSED')) AS shifts_closed,
          COALESCE(SUM(a.total_hours) FILTER (WHERE a.status IN ('CLOSED', 'AUTO_CLOSED')),0) AS worked_net_hours,
          COALESCE(SUM(a.paid_hours) FILTER (WHERE a.status IN ('CLOSED', 'AUTO_CLOSED')),0) AS paid_hours
        FROM attendance a
      ...
      pay AS (
        ...
        WHERE a.scheduled_start IS NOT NULL
          AND a.scheduled_start::date BETWEEN $1::date AND $2::date
          AND a.status IN ('CLOSED', 'AUTO_CLOSED')
        GROUP BY p.user_id
      )
      SELECT
        u.id AS user_id,
